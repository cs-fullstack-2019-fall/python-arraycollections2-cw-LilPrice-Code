...
def pro1():
    peray = ["Kenn", "Kevin", "Erin", "Autumn"]
    f = range(1,3)
    for x in f:
        print(peray[x])
    print(peray[2:4])
    peray.insert(2,"Chuck")
    print(peray)
    peray.pop(1)
    print(peray)
...
def pro3():
    array = ['GOOD_DATA','DECENT_DATA','BAD_DATA','DECENT_DATA','GOOD_DATA','BAD_DATA','GOOD_DATA','DECENT_DATA','BAD_DATA','GOOD_DATA']
    print(len(array))
    array.remove('BAD_DATA') # !! : you should be able to do this without knowing how many BAD_DATA items there are in the array
    array.remove('BAD_DATA')
    array.remove('BAD_DATA')
    print(len(array))
...
def pro4():
    score = [21,14,6,8,28,42,21]
    print(f"The SUM of the numbers is {sum(score)}")
    print(f'The MAX of the numbers is {max(score)}')
    print(f'The MIN of the numbers is {min(score)}')
